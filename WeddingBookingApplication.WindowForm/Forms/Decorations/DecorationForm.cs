...
namespace WeddingBookingApplication.WindowForm.Forms.Decorations;

/// <summary>
/// Full CRUD panel for Decoration Packages.
///
/// Responsive changes:
...
/// </summary>
public class DecorationForm : UserControl
{
    private readonly AppDbContext _db        = DbContextFactory.Create();
    private DecorationService     _service   = null!;
    ...
    public DecorationForm()
    {
        _service   = new DecorationService(_db);
    ...
    private void BuildUI()
    {
        ...
        // ── Header ──────────────────────────────────────────────────────
        var header = UITheme.BuildPageHeader("🌸", "Decorations", "Manage decoration packages");
        ...
        // Critical: prevent auto-generated columns
        ...
        _grid.Columns.AddRange(
            TxtCol("DecorationPackageId", "ID", 55),
            TxtCol("VendorId", "Vendor ID", 80),
            TxtCol("PackageName", "Package", 180),
        ...
        UITheme.AddDetailRow(tbl, "Package Name *",  _txtName);
    ...
    private void LoadGrid()
    {
        try
        {
            var pkgs = _service.GetDecorationPackages();
    ...
    private void Grid_SelectionChanged(object? sender, EventArgs e)
    {
        ...
        var id  = Convert.ToInt32(_grid.SelectedRows[0].Cells["DecorationPackageId"].Value);
        var pkg = _service.GetDecorationPackage(id);
        if (pkg is null) return;

        _selectedId         = id;
        _detailHeading.Text = "Edit Package";
    ...
    {
        _selectedId         = null;
        _detailHeading.Text = isNew ? "New Package" : "Select a package";
        if (_cbVendor.Items.Count > 0) _cbVendor.SelectedIndex = 0;
    ...
    private void BtnSave_Click(object? sender, EventArgs e)
    {
        ...
        { SetStatus("Vendor and Package Name are required.", UITheme.Danger); return; }

        if (_selectedId.HasValue)
        {
            var res = _service.UpdateDecorationPackage(_selectedId.Value, new DecorationUpdateRequestModel
            {
        ...
        {
            var res = _service.CreateDecorationPackage(new DecorationCreateRequestModel
            {
                VendorId = vendor.Id, PackageName = _txtName.Text.Trim(),
                Price = _nudPrice.Value, Description = _txtDesc.Text.Trim(), IsActive = _chkActive.Checked
            });
            SetStatus(res.IsSuccess ? $"✔  Package #{res.DecorationPackageId} created." : $"✘  {res.Message}",
    ...
    private void BtnDelete_Click(object? sender, EventArgs e)
    {
        ...
        if (MessageBox.Show("Deactivate this decoration package?", "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) != DialogResult.Yes) return;
        var res = _service.DeleteDecorationPackage(_selectedId.Value);
